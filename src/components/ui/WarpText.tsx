...
void main() {
  ...
  if (uRefraction > 0.0001) {
    ...
    vec2 split = splitDir * uRefraction * 0.16 * (0.35 + lens * 1.5);
    ...
    fragColor = sampleText(displaced);
  }
}
`;
...
const WarpText = ({
  ...
  useEffect(() => {
    ...
    const loop = (now: number): void => {
      ...
      const damping = pointer.activeTarget > 0 ? 0.16 : 0.035;

      pointer.x += (targetX - pointer.x) * damping;
      pointer.y += (targetY - pointer.y) * damping;
      pointer.active += ((pointer.activeTarget > 0 ? 1 : 0.18) - pointer.active) * 0.08;
  ...
  return (
    <div
      ref={containerRef}
      className={`relative block min-h-[220px] w-full overflow-hidden isolate pointer-events-auto ${className}`.trim()}
